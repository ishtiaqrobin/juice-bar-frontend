...
export default function FeaturedPage() {
    ...
    const apiUrl = `/api/featured-options?includeInactive=${showInactive}`;
    const { data: featuredOptions = [], isLoading, mutate, error } = useSWR(apiUrl, fetcher, {
    ...
    const [deleting, setDeleting] = React.useState({ open: false, featuredId: null, name: '' });
    ...
    const handleAddSubmit = async (e) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(error.error || 'Failed to create featured option');
            }
    ...
    const saveEdit = async () => {
        ...
        try {
            const res = await fetch(`/api/featured-options/${editingId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            if (!res.ok) {
                const error = await res.json();
                throw new Error(error.error || 'Failed to update featured option');
            }
    ...
    const doDelete = async () => {
        ...
        try {
            const res = await fetch(`/api/featured-options/${deleting.featuredId}`, {
                method: 'DELETE'
            });

            if (!res.ok) {
                const error = await res.json();
                throw new Error(error.error || 'Failed to delete featured option');
            }
        ...
        <div className="mx-auto w-full space-y-6">
            ...
            <div className="flex items-center gap-2">
                <Switch
                    ...
                    className="hover:cursor-pointer"
                />
                <label className="text-sm font-medium">Show inactive featured options</label>
            </div>

            {error ? (
                <div className="text-center text-red-600">Failed to load featured options</div>
            ) : (
                            ...
                            {isLoading ? (
                                ...
                                <TableRow>
                                    <TableCell colSpan={4} className="text-center py-8 text-gray-600">
                                        {query ? 'No featured options found matching your search' : 'No featured options found'}
                                    </TableCell>
                                </TableRow>
                                ...
                                <TableRow key={featured.id} className="align-middle">
                                    ...
                                    <TableCell>
                                        <span className={`text-sm font-medium ${featured.isActive ? 'text-green-600' : 'text-red-600'}`}>
            ...
            <Dialog open={addOpen} onOpenChange={(open) => { if (!open) setAddOpen(false); }}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>Create a new featured option for your products.</DialogDescription>
                    </DialogHeader>
            ...
            <Dialog open={editOpen} onOpenChange={(open) => { if (!open) cancelEdit(); }}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>Update featured option information.</DialogDescription>
                    </DialogHeader>
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This action cannot be undone. This will permanently delete the featured option <span className="font-semibold text-primary">"{deleting.name}"</span>.
                            If any products are using this featured option, you won't be able to delete it until you remove or change the featured status of those products.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
