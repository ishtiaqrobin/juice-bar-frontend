...
export default function BannersPage() {
    ...
    const apiUrl = `/api/banners?includeInactive=${showInactive}&adminOnly=true`;
    const { data: banners = [], isLoading, mutate, error } = useSWR(apiUrl, fetcher, {
    ...
    const [deleting, setDeleting] = React.useState({ open: false, bannerId: null, text: '' });
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (isEdit) {
                    ...
                    setAddForm(prev => ({ ...prev, image: result.url }));
                }
    ...
    const doDelete = async () => {
        ...
        try {
            const res = await fetch(`/api/banners/${deleting.bannerId}`, {
                method: 'DELETE'
            });
    ...
    return (
        <div className="mx-auto w-full space-y-6">
            ...
            {error ? (
                <div className="text-center text-red-600">Failed to load banners</div>
            ) : (
                            ...
                            {isLoading ? (
                                ...
                                <TableRow>
                                    <TableCell colSpan={6} className="text-center py-8 text-gray-600">
                                        {query ? 'No banners found matching your search' : 'No banners found'}
                                    </TableCell>
                                </TableRow>
                                ...
                                <TableRow key={banner.id} className="align-middle">
                                    ...
                                    <TableCell>
                                        <span className={`text-sm font-medium ${banner.isActive ? 'text-green-600' : 'text-red-600'}`}>
            ...
            <Dialog open={addOpen} onOpenChange={(open) => { if (!open) setAddOpen(false); }}>
                <DialogContent className="">
                    <DialogHeader>
                        ...
                        <DialogDescription>Create a new banner for your carousel.</DialogDescription>
                    </DialogHeader>
                    <form onSubmit={handleAddSubmit} className="space-y-4">
                        ...
                        <div className="space-y-1.5">
                            <label className="text-sm font-medium text-gray-800">Text/Offer</label>
                            <Input
                                value={addForm.text}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="flex items-center gap-2 pt-2 md:pt-6">
                                <Switch
                                    ...
                                    onCheckedChange={(checked) => setAddForm((p) => ({ ...p, isActive: checked }))}
                                    className="hover:cursor-pointer"
                                />
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This action cannot be undone. This will permanently delete the banner <span className="font-semibold text-primary">"{deleting.text}"</span> and its image file.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
