...
export function useAuth() {
    ...
    await authClient.signIn.email({
      ...
      fetchOptions: {
        onSuccess: (ctx) => {
          ...
          // ✅ router.refresh() সরিয়ে দাও — push নিজেই page reload করবে
          // refresh() middleware কে আগেই trigger করত, তখন cookie set হয়নি
          if (ctx.data.user.role === Roles.admin) {
            router.push("/admin-dashboard");
          } else {
            router.push("/");
  ...
  const logout = async () => {
    try {
      await authClient.signOut();
      toast.success("Logged out successfully");
      // ✅ setTimeout এবং window.location ঠিক আছে, তবে এভাবে cleaner
      window.location.href = "/login";
    } catch (error) {
      console.error("Logout error:", error);
      toast.error("Logout failed");
